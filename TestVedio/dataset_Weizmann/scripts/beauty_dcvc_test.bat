...
:: 视频文件名
set filename=Beauty_1920x1080_120fps_420_8bit_YUV
...
:: 检查GPU状态
echo 检查GPU状态...
python -c "import torch; print('GPU可用' if torch.cuda.is_available() else 'GPU不可用'); print('GPU设备:', torch.cuda.get_device_name(0) if torch.cuda.is_available() else '无')" > "%RESULTS_DIR%\dcvc\gpu_info.txt"
...
for %%q in (%qp_values%) do (
    ...
    python "%DCVC_PATH%\test_video.py" ^
        --input "%INPUT_DIR%\%filename%.yuv" ^
        --output "%OUTPUT_DIR%\dcvc\%filename%_q%%q.bin" ^
        --recon "%RECON_DIR%\dcvc\%filename%_q%%q_reconstructed.yuv" ^
        --qp %%q ^
        --width 1920 ^
        --height 1080 ^
        --fps 120 ^
